...
- **Job portals:** "The framework's built-in job search runs on LinkedIn (works for any US city or remote). Are there other sites you use — Indeed, Dice, Wellfound, Built In, specific company career pages?" Note: additional boards are reached via WebSearch site-queries in `search-queries.md`; dedicated CLI integrations can be added later using the pattern in `.agents/skills/linkedin-search/`.
